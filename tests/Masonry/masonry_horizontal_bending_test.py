...
class TestUnreinforcedMasonryHorizontalBending:
    def test_horizontal_bending_1(self):
        ...
        wall = unreinforced_masonry.Clay(length=3000, height=3*86-20, thickness=110, fuc = 20, mortar_class=3,bedding_type=True)
    ...
    def test_horizontal_bending_2(self):
        ...
        Mch = 2 * phi * kp * (math.sqrt(fmt)) * (1 + fd/fmt) * Zd = 2 * 0.6 * 1 * 0.447 *1 * 5,290,000 = 2.84 KNm
        Mch = 4 * phi * kp * (math.sqrt(fmt)) * Zd = 4 * 0.6 * 1 * 0.447 * 5,290,000 = 5.68 KNm
        Mch = phi * (0.44 * fut * Zu + 0.56 * fmt * Zp) = 0.6 * (0.44 * 0.8MPa * 5,290,000 mm3 + 0.56 * 0.2MPa * 5,290,000 mm3) = 1.47 KNm
        ...
        wall = unreinforced_masonry.Clay(length=2000, height=600, thickness=230, fmt = 0.2, fuc = 20, mortar_class=4,bedding_type=True)
